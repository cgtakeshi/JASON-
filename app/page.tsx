...
const experiences = [
  ...
  { period:"2017.09 — 2020.04", companyZh:"淘乐网络", companyEn:"Taole Network", logo:"/logo-taole.png", mark:"TL", roleZh:"主美 / 代号 S 项目主设计", roleEn:"Lead Artist / Project S Lead Designer", projectZh:"桃花源记 · 少年仙界传", projectEn:"Tales of Peach Blossom · Young Immortal Realm", descZh:"主导国风 Q 版回合制项目的美术风格孵化、设计与制作标准、项目目标及计划，并搭建美术中台与职级晋升体系。", descEn:"Led visual style development for stylized Chinese turn-based games, defining design and production standards, project goals and schedules while building a shared art platform and career progression system." },
...
const experienceLinks: Record<string, string> = {
  ...
  "冰川网络": "https://store.steampowered.com/app/3362120/Elysium_ExpanseBDMMORPG/",
  "淘乐网络": "https://www.taptap.cn/app/175348?os=android",
  "网易签约画师 / 个人工作室": "https://dhxy.163.com/index.html",
};
  ...
  return (
    <main>
      ...
      <section className="about section shell" id="experience">
        ...
        <div className="timeline">
          {experiences.map((item) => <article className="timeline-row" data-reveal data-tilt key={item.companyZh}>
            ...
            <div><h3>{isEnglish ? item.companyEn : item.companyZh}</h3><div className="timeline-project"><p>{isEnglish ? item.projectEn : item.projectZh}</p>{experienceLinks[item.companyZh] && <a className="timeline-store-link" href={experienceLinks[item.companyZh]} target="_blank" rel="noreferrer" aria-label={isEnglish ? `Open ${item.projectEn} store page` : `打开 ${item.projectZh} 商店页面`}><span aria-hidden="true">↗</span></a>}</div></div><div><strong>{isEnglish ? item.roleEn : item.roleZh}</strong><p>{isEnglish ? item.descEn : item.descZh}</p></div>
          </article>)}
        </div>
      </section>
      ...
      {activeStrength && activeStrengthSlide && <div className="strength-browser" role="dialog" aria-modal="true" aria-label={`${isEnglish ? activeStrength.titleEn : activeStrength.titleZh} ${copy.strengthBrowser}`} onMouseDown={(event) => { if (event.target === event.currentTarget) setOpenStrength(null); }}>
        <header className="strength-browser-head"><div><span>CORE STRENGTH {activeStrength.no}</span><h2>{isEnglish ? activeStrength.titleEn : activeStrength.titleZh}</h2><p>{activeStrength.en} · PPT {activeStrengthSlide.page}</p></div><button type="button" className="modal-close-button" onClick={() => setOpenStrength(null)} aria-label={copy.closeStrength}>{copy.close.toUpperCase()} ×</button></header>
